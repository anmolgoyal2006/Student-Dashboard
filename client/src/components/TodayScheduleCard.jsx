        ...
        <>
          ...
          {(() => {
            ...
            return displayClasses.map((cls, idx) => {
              ...
              const optimisticKey = `${cls.subjectId}_${slotIndex}`;
              const optimisticStatus = optimisticMarks[optimisticKey] || null;
              const subjectRecords = todayRecordsBySubject[key] || [];
              const recordStatus = subjectRecords[slotIndex]?.status || null;
              // Optimistic takes priority over record (prevents stale flash)
              const status = optimisticStatus || recordStatus;
              return (
