...
function startCollectorScheduler() {
  ...
  cron.schedule('*/15 * * * *', lockedJob('runCollectors', runCollectors), {
    scheduled: true,
    timezone: 'Asia/Kolkata',
  });

  // Refresh recommendations every 6 hours
  cron.schedule('0 */6 * * *', lockedJob('refreshAllRecommendations', refreshAllRecommendations), {
    scheduled: true,
    timezone: 'Asia/Kolkata',
  });

  // Check for due reminders every hour
  cron.schedule('0 * * * *', lockedJob('sendDueReminders', sendDueReminders), {
    scheduled: true,
    timezone: 'Asia/Kolkata',
  });
  ...
  lockedJob('runCollectors', runCollectors)();
}
